...
def groupSum5(start, nums, target):
  if (start >= len(nums)):
    ...
    if (start < len(nums) - 1 and nums[start] == 1):
      return groupSum5(start + 2, nums, target - nums[start])
    else:
      return groupSum5(start + 1, nums, target - nums[start])
  elif groupSum5(start + 1, nums, target - nums[start]):
    return True
  else:
...
def splitArrayHelper(start, nums, sum1, sum2):
  if (start >= len(nums)):
    return sum1 == sum2
  else:
    return splitArrayHelper(start+1, nums, sum1+nums[start],sum2) or splitArrayHelper(start+1, nums, sum1, sum2+nums[start])
...
def splitOdd10Helper(start,nums,sum1,sum2):
  if (start >= len(nums)):
    return (sum1 % 10 == 0 and sum2 % 2 == 1) or (sum1 %2 == 1 and sum2 % 10 == 0)
  else:
    return splitOdd10Helper(start+1,nums,sum1+nums[start],sum2) or splitOdd10Helper(start+1,nums,sum1,sum2+nums[start])
...
def split53Helper(start,nums,sum1,sum2):
  if (start >= len(nums)):
    return sum1 == sum2
  elif (nums[start] % 5 == 0):
    ...
    return split53Helper(start+1,nums,sum1+nums[start],sum2) or split53Helper(start+1,nums,sum1,sum2+nums[start])
